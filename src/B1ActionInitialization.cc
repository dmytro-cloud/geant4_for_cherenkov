...
B1ActionInitialization::B1ActionInitialization(double energy_, double length_)
 ...
 length(length_)
{
  ...
  file.open(outputFileName, std::ios::trunc);
  file << "ID\tX\tY\tZ\tE\n";
  file.close();
...
{
  B1RunAction* runAction = new B1RunAction(energy, length);
...
void B1ActionInitialization::Build() const
{
  ...
  SetUserAction(new B1SteppingAction(eventAction, energy, length));
